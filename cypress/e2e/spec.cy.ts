describe('Navigation Test', () => {
  it('Visits the initial project page and navigates', () => {
    ...
    cy.get('button').contains('Mode').then(($btn) => {
      const mode = $btn.text()
      if (mode === 'Dark Mode') {
        cy.get('.header').find('img').should('be.visible').and('not.have.class', 'dark-img')
    ...
    cy.get('.footer').find('[href="https://www.linkedin.com/in/daniel-teel-a6465017b"]').should('exist').should('have.attr', 'target', '_blank')
    cy.get('.footer').find('[href="https://github.com/TheManOfTeel"]').should('exist').should('have.attr', 'target', '_blank')
  })
})
